...
void compile_statement1(int st_no, expr x) {
  ...
  if (unit_ctl_level > 0 &&
      ...
      && EXPR_CODE(x) != F_PRAGMA_STATEMENT && EXPR_CODE(x) != F_COMMENT_LINE) {
    /* otherwise error */
  ...
  case F08_SUBMODULE_STATEMENT: /* (F08_SUBMODULE_STATEMENT submodule_name
    ...
    begin_procedure();
    declare_procedure(CL_SUBMODULE, EXPR_ARG1(x), type_MODULE, NULL, NULL, NULL,
                      NULL);
  ...
  case F_SUBROUTINE_STATEMENT:
    ...
    begin_procedure();
    declare_procedure(CL_PROC, EXPR_ARG1(x), subroutine_type(), EXPR_ARG2(x),
  ...
  case F_FUNCTION_STATEMENT: {
    ...
    declare_procedure(CL_PROC, EXPR_ARG1(x), function_type(tp), EXPR_ARG2(x),
                      prefixes, EXPR_ARG4(x), EXPR_ARG5(x));
    break;
  }
  ...
  case F_ENDDO_STATEMENT: {
    ...
    if (CTL_TYPE(ctl_top) == CTL_OMP) {
      if (CTL_OMP_ARG_DIR(ctl_top) == OMP_F_PARALLEL_DO) {
        ...
        CTL_BLOCK(ctl_top) = OMP_FOR_pragma_list(dclause, CURRENT_STATEMENTS);
        EXPR_LINE(CTL_BLOCK(ctl_top)) = EXPR_LINE(CTL_OMP_ARG(ctl_top));
  ...
  case F03_SELECTTYPE_STATEMENT: {
    ...
    if (EXPR_HAS_ARG3(x)) {
      ...
      expv tmp =
          expv_sym_term(IDENT, ID_TYPE(associate_name), ID_SYM(associate_name));

      st = list4(F03_SELECTTYPE_STATEMENT, v, NULL, EXPR_ARG2(x), tmp);
    } else {
      if (selector == NULL) {
        error_at_node(
            x, "If selector is an expression, associate name is required");
      }
...
static void compile_exec_statement(expr x) {
  ...
  case F_LET_STATEMENT: /* (F_LET_STATEMENT lhs rhs) */
    ...
    if (CURRENT_STATE == OUTSIDE) {
      ...
      declare_procedure(CL_MAIN,
                        make_enode(IDENT, find_symbol(NAME_FOR_NONAME_PROGRAM)),
    ...
    case F_ARRAY_REF: /* for a statement function because it looks like an array
                         reference. */
...
void fix_type(ID id) {
  ...
  if (VAR_IS_UNCOMPILED(id) == TRUE) {
    ...
    if (tp != NULL) {
      ...
      ID_TYPE(id) = NULL;
      compile_type_decl(NULL, tp, list1(LIST, EXPR_ARG1(x)), EXPR_ARG2(x));
    }
...
static void update_procedure_variable(ID id, const ID target, int is_final) {
  ...
  if (!FUNCTION_TYPE_HAS_EXPLICT_INTERFACE(
    ...
    if (is_final) {
      error_at_id(VAR_REF_PROC(id), "%s should have an explicit interface",
                  SYM_NAME(ID_SYM(id)));
    } else {
      return;
    }
      ...
      if (target != NULL) {
        ...
        if (debug_flag) {
          fprintf(debug_fp, "bind %s to %s%%%s\n", SYM_NAME(ID_SYM(target)),
                  SYM_NAME(ID_SYM(TYPE_TAGNAME(tp))), SYM_NAME(ID_SYM(mem)));
        }
...
static void end_declaration() {
   ...
   */
  FOREACH_ID(ip, LOCAL_SYMBOLS) { fix_pointer_pointee_recursive(ID_TYPE(ip)); }
  ...
  FOREACH_ID(ip, LOCAL_SYMBOLS) {
    ...
    if (tp == NULL && ID_CLASS(ip) == CL_PROC && PROC_CLASS(ip) == P_EXTERNAL) {
      ...
      TYPE_ATTR_FLAGS(FUNCTION_TYPE_RETURN_TYPE(tp)) |=
          (TYPE_ATTR_FLAGS(ip) &
           ~(TYPE_ATTR_SAVE | TYPE_ATTR_BIND | TYPE_ATTR_PUBLIC |
             TYPE_ATTR_PRIVATE | TYPE_ATTR_INTENT_INOUT | TYPE_ATTR_INTENT_IN |
             TYPE_ATTR_INTENT_OUT | TYPE_ATTR_OPTIONAL | TYPE_ATTR_VALUE |
             TYPE_ATTR_VOLATILE | TYPE_ATTR_ASYNCHRONOUS));
    }
    ...
    if (ID_CLASS(ip) != CL_MODULE) {
      ...
      for (check = type_attr_checker; check->flag; check++) {
        if (TYPE_ATTR_FLAGS(tp) & check->flag) {
          ...
          uint32_t a = TYPE_ATTR_FLAGS(tp) & ~check->acceptable_flags & mask;
          if (debug_flag) {
            fprintf(debug_fp,
                    "ID '%s' attr 0x%08x : "
                    "matches 0x%08x ('%s'), "
      ...
      if (TYPE_TYPE_PARAM_VALUES(tp)) {
        ...
        FOR_ITEMS_IN_LIST(lp, TYPE_TYPE_PARAM_VALUES(tp)) {
          if (EXPV_CODE(LIST_ITEM(lp)) == F08_LEN_SPEC_COLON) {
            if (!TYPE_IS_POINTER(tp) && !TYPE_IS_POINTER(ID_TYPE(ip)) &&
                !TYPE_IS_ALLOCATABLE(tp) && !TYPE_IS_ALLOCATABLE(ID_TYPE(ip))) {
              error_at_id(ip, "type parameter value ':' should be used "
  ...
  FOREACH_ID(ip, LOCAL_SYMBOLS) {
    if (PROC_CLASS(ip) == P_EXTERNAL && PROC_EXT_ID(ip) == NULL) {
      ...
      EXT_ID ep = new_external_id_for_external_decl(ID_SYM(ip), ID_TYPE(ip));
      PROC_EXT_ID(ip) = ep;
    }
  }
    ...
    if (tp) {
      if (TYPE_IS_OPTIONAL(tp) && !(ID_IS_DUMMY_ARG(ip))) {
        ...
        error_at_id(ip,
                 ...
                 !(IS_ARRAY_TYPE(tp) &&
                   (TYPE_IS_POINTER(tp) || TYPE_IS_ARRAY_ASSUMED_SHAPE(tp)))) {
        error_at_id(ip, "Only an array pointer or an assumed-shape array can "
                        "have the CONTIGUOUS attribute");
      } else if (IS_PROCEDURE_TYPE(tp) && TYPE_IS_PROCEDURE(tp)) {
        if (ID_STORAGE(ip) != STG_ARG) {
          if (VAR_INIT_VALUE(ip) && !TYPE_IS_POINTER(tp)) {
            error_at_id(ip, "PROCEDURE variable with an inilial pointer should "
                            "have the POINTER attribute");
          }
          if (TYPE_IS_OPTIONAL(tp)) {
            error_at_id(
                ip, "PROCEDURE variable should not have the OPTINAL attribute");
          }
          if (TYPE_IS_INTENT_IN(tp) || TYPE_IS_INTENT_OUT(tp) ||
              TYPE_IS_INTENT_INOUT(tp)) {
            error_at_id(
                ip, "PROCEDURE variable should not have the INTENT attribute");
          }
        }
      }
        ...
        if (is_pure) {
          if (is_subroutine) {
            if (ID_IS_DUMMY_ARG(ip) && !TYPE_IS_POINTER(tp) &&
              ...
              error_at_id(ip, "nonpointer argument of PURE function "
                              "should have INTENT(*) or VALUE attribute");
            }
          } else {
            if (ID_IS_DUMMY_ARG(ip) && !TYPE_IS_POINTER(tp) &&
                !TYPE_IS_INTENT_IN(tp) && !TYPE_IS_VALUE(tp)) {
              error_at_id(ip, "nonpointer argument of PURE subroutine "
...
static EXT_ID procedure_defined(ID f_id, EXT_ID unit_ctl_procs[],
  ...
  if (f_id == NULL || ID_CLASS(f_id) != CL_PROC) {
    if (debug_flag)
      warning("unexpected id '%s' in '%s', id is not procedure", ID_NAME(f_id),
              __func__);
    return NULL;
  }
    ...
    if (contained_proc == NULL) {
      ...
      if (external_proc == NULL) {
        if (ID_TYPE(id) != NULL && ID_STORAGE(id) != STG_EXT &&
          ...
          error_at_id(id, "%s is used as an explicit interface but not defined",
                      SYM_NAME(ID_SYM(id)));
          continue;
        } else {
          external_proc =
...
static void check_labels_in_block(BLOCK_ENV block) {
  ...
  FOREACH_ID(id, BLOCK_LOCAL_LABELS(block)) {
    if (LAB_TYPE(id) != LAB_UNKNOWN && LAB_IS_USED(id) && !LAB_IS_DEFINED(id)) {
  ...
  FOREACH_ID(id, id_list) {
    ...
    if (IS_PROCEDURE_TYPE(ID_TYPE(id)) && TYPE_IS_PROCEDURE(ID_TYPE(id))) {
      ...
      if (ID_TYPE(target) == NULL ||
        ...
        if (is_final)
          error_at_id(id, "Interface %s does not have explict interface",
                      SYM_NAME(ID_SYM(VAR_REF_PROC(id))));
        continue;
      }
      ...
      if (stp != NULL && !check_tbp_pass_arg(stp, ID_TYPE(id), ftp)) {
        fprintf(stderr, "Interface %s does not have a PASS argument",
                SYM_NAME(ID_SYM(target)));
        if (is_final)
          error_at_id(id, "Interface %s does not have a PASS argument",
                      SYM_NAME(ID_SYM(target)));
        continue;
      }
  ...
  FOREACH_STRUCTDECLS(tp, LOCAL_STRUCT_DECLS) {
      ...
      FOREACH_ID(binding, TBP_BINDING(mem)) {
        ...
        TBP_BINDING_ATTRS(bindto) |=
            ...
            (TYPE_BOUND_PROCEDURE_IS_OPERATOR |
             ...
             TYPE_BOUND_PROCEDURE_IS_ASSIGNMENT | TYPE_BOUND_PROCEDURE_WRITE |
             TYPE_BOUND_PROCEDURE_READ | TYPE_BOUND_PROCEDURE_FORMATTED |
    ...
    FOREACH_TYPE_BOUND_PROCEDURE(tbp, tp) {
      ...
      if (parent) {
        ...
        if (!type_bound_procedure_types_are_compatible(ID_TYPE(tbp),
                                                       ID_TYPE(parent_tbp))) {
    ...
    if ((final = find_struct_member(tp, sym)) != NULL) {
      ...
      FOREACH_ID(binding, TBP_BINDING(final)) {
        ...
        if (TBP_BINDING_ATTRS(fin) & TYPE_BOUND_PROCEDURE_IS_FINAL) {
          error("FINAL subroutine %s used duplicately", SYM_NAME(ID_SYM(fin)));
          return;
        }
...
static void end_procedure() {
    ...
    FOREACH_EXT_ID(intr, LOCAL_INTERFACES) {
      ...
      FOREACH_EXT_ID(intrDef, EXT_PROC_INTR_DEF_EXT_IDS(intr)) {
        if (EXT_PROC_IS_MODULE_PROCEDURE(intrDef)) {
          ...
          if (id != NULL && ID_CLASS(id) == CL_PROC && ID_IS_OFMODULE(id)) {
  ...
  if (auto_save_attr_kb >= 0 &&
    ...
    FOREACH_ID(id, LOCAL_SYMBOLS) {
      ...
      if (ID_STORAGE(id) != STG_AUTO || ID_CLASS(id) == CL_PARAM || t == NULL ||
          TYPE_IS_SAVE(t) || IS_PROCEDURE_TYPE(t))
        continue;
  ...
  FOREACH_ID(id, LOCAL_SYMBOLS) {
    ...
    if ((ID_CLASS(id) == CL_PROC && PROC_CLASS(id) == P_THISPROC) ||
      ...
      if (unit_ctl_level != 0) {
        ...
        id_in_parent = find_ident_head(ID_SYM(id),
                                       UNIT_CTL_LOCAL_SYMBOLS(PARENT_UNIT_CTL));
  ...
  if (CURRENT_PROC_CLASS == CL_MODULE) {
    if (!export_module(current_module_name, LOCAL_SYMBOLS, LOCAL_USE_DECLS)) {
#if 0
...
static void solve_use_assoc_conflict(ID id, ID mid) {
  if (ID_SEEM_GENERIC_PROCEDURE(id) && ID_SEEM_GENERIC_PROCEDURE(mid)) {
    ...
    /* NOTE:
     * Generic functions with functions with different type of arguments is not
     * conflict. Generic function occurres a conflict if it cotains functions
     * with same type of arguments, but the current type system couldn't detect
     * it.
     */
  ...
  if (!id->use_assoc) {
    ...
    if ((id->use_assoc->module_name == mid->use_assoc->module_name) &&
      ...
      if (debug_flag) {
        fprintf(debug_fp,
                "duplicate use assoc symbol '%s' (original '%s') from module "
                "'%s'\n",
                SYM_NAME(ID_SYM(mid)), SYM_NAME(mid->use_assoc->original_name),
      ...
      if (debug_flag) {
        fprintf(debug_fp,
                ...
                " - original '%s' from module '%s'\n",
                SYM_NAME(ID_SYM(id)), SYM_NAME(id->use_assoc->original_name),
    ...
    if (args != NULL) {
      ...
      FOREACH_USE_ARG(arg, args) {
        ...
        if (arg->local != ID_SYM(mid))
          continue;
        found = TRUE;
        import_module_id(mid, &LOCAL_SYMBOLS, &last_id, &LOCAL_STRUCT_DECLS,
                         &sttail, arg->use, wrap_type, fromParentModule);
        arg->used = TRUE;
      }
      if (!found && !isOnly) {
        import_module_id(mid, &LOCAL_SYMBOLS, &last_id, &LOCAL_STRUCT_DECLS,
                         &sttail, NULL, wrap_type, fromParentModule);
      }
    } else {
      if (!isOnly) {
        wrap_type = TRUE;
  ...
  FOR_ITEMS_IN_LIST(lp, EXPR_ARG1(x)) {
    ...
    if (EXPR_CODE(r) == F_SET_EXPR) {
      ...
      if (strcmp(SYM_NAME(EXPR_SYM(kwd)), "stat") == 0) {
        ...
        if (vstat == NULL ||
            (EXPR_CODE(vstat) != F_VAR && EXPR_CODE(vstat) != ARRAY_REF &&
        ...
        if (verrmsg == NULL ||
            (EXPR_CODE(verrmsg) != F_VAR && EXPR_CODE(verrmsg) != ARRAY_REF &&
      ...
      case ARRAY_REF:
      case XMP_COARRAY_REF:
        if (isVarSetTypeAttr(ev, TYPE_ATTR_POINTER | TYPE_ATTR_ALLOCATABLE) ==
            FALSE) {
...
static void compile_CALL_member_procedure_statement(expr x) {
  ...
  if (!TYPE_IS_IMPLICIT(tp) && !FUNCTION_TYPE_IS_GENERIC(tp) && !IS_SUBR(tp) &&
...
void compile_CALL_subroutine_statement(expr x) {
  ...
  if (ID_TYPE(id) != NULL) {
    if (IS_FUNCTION_TYPE(ID_TYPE(id)) && TYPE_IS_USED_EXPLICIT(ID_TYPE(id))) {
  ...
  if (ID_IS_DUMMY_ARG(id)) {
    ...
    if (v == NULL && PROC_CLASS(id) == P_INTRINSIC) {
      ...
      /* NOTE: DO NOT 'TYPE_SET_EXTERNAL(id)', this is not an explicit exteranl
       * function  */
...
static int pointer_assignable(expr x, expv vPointer, expv vPointee,
  ...
  if (TYPE_IS_READONLY(vPtrTyp) &&
    ...
    if (x)
      error_at_node(x, "'%s' is PROTECTED.", SYM_NAME(EXPR_SYM(EXPR_ARG1(x))));
    return FALSE;
  }
...
static void compile_POINTER_SET_statement(expr x) {
  ...
  if (nArgs != 2) {
    fatal("%s: Invalid arguments number, expect 2 but %d.", __func__, nArgs);
    return;
  }
  ...
  if (IS_PROCEDURE_TYPE(vPtrTyp)) {
    if (EXPR_CODE(vPointee) == F_VAR) {
      ...
      if (!IS_PROCEDURE_TYPE(vPteTyp)) {
        ...
        } else if (IS_FUNCTION_TYPE(vPteTyp) && TYPE_IS_IMPLICIT(vPteTyp) &&
...
static void compile_OPTIONAL_statement(expr x) {
  ...
  FOR_ITEMS_IN_LIST(lp, EXPR_ARG1(x)) {
    ...
    if (!ID_IS_DUMMY_ARG(id)) {
      error_at_node(x, "\"%s\" is not a dummy argument.", SYM_NAME(ID_SYM(id)));
      continue;
    }
    ...
    FOR_ITEMS_IN_LIST(lp, UNIT_CTL_IMPLICIT_DECLS(unit_ctls[i])) {
      ...
      if (EXPR_CODE(v) == IDENT)
        set_implicit_type_uc(CURRENT_UNIT_CTL, EXPV_TYPE(v),
                             *(SYM_NAME(EXPR_SYM(v))), *(SYM_NAME(EXPR_SYM(v))),
                             TRUE);
      else
        set_implicit_type_uc(CURRENT_UNIT_CTL, EXPV_TYPE(v),
...
static int compile_stat_args(expv st, expr x, int expect_acquired_lock) {
  ...
  FOR_ITEMS_IN_LIST(lp, x) {
    ...
    } else if (expect_acquired_lock && strcmp("acquired_lock", keyword) == 0) {
...
static int type_is_LOCK_TYPE(TYPE_DESC tp) {
  ...
  if (tagname != NULL && ID_USEASSOC_INFO(tagname) != NULL &&
      strcmp("lock_type", SYM_NAME(ID_USEASSOC_INFO(tagname)->original_name)) ==
          0 &&
...
static expv compile_forall_header(expr x) {
  ...
  FOR_ITEMS_IN_LIST(lp, triplets) {
    ...
    if (low_limit == NULL ||
        (!IS_INT(EXPV_TYPE(low_limit)) && !IS_GNUMERIC(EXPV_TYPE(low_limit)) &&
         !IS_GNUMERIC_ALL(EXPV_TYPE(low_limit)))) {
      error("invalid expression");
    }
    if (top_limit == NULL ||
        (!IS_INT(EXPV_TYPE(top_limit)) && !IS_GNUMERIC(EXPV_TYPE(top_limit)) &&
...
static void compile_ASSOCIATE_statement(expr x) {
   ...
   *      2 REAL :: Y = 2.5
   *      3 ASSOCIATE ( Y => X, Z => Y )
   *      4   !                This `Y` refers `Y` in the line 2, not `Y` in the
   * line 3 4   PRINT *, Z !=> 2.5 5 END ASSOCIATE
   *
