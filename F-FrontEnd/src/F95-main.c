...
char *argv[];
{
  ...
  while (argc > 0 && argv[0] != NULL) {
    if (argv[0][0] != '-' && argv[0][0] != '\0') {
      ...
      if (flag_force_fixed_format != -1)
        cmd_warning("it seems to be set both of -force-fixed-format and "
                    "-force-free-format.");
      ...
      if (includeDirvI < 256) {
        ...
        cmd_error_exit("over the maximum include search dir. vector, %d",
                       MAXINCLUDEDIRV);
      }
      ...
      if (modincludeDirvI < MAXMODINCLUDEDIRV) {
        ...
        cmd_error_exit("over the maximum module include search dir. vector, %d",
                       MAXMODINCLUDEDIRV);
      }
      ...
      if (flag_force_fixed_format == 1) {
        cmd_warning("no need option for enable c comment(-force-c-comment) in "
  ...
  if (source_file_name == NULL) {
    ...
    if (flag_force_fixed_format == -1) { /* unset?  */
      ...
      if (dotPos != NULL &&
          (strcasecmp(dotPos, ".f") == 0 || strcasecmp(dotPos, ".f77") == 0)) {
