...
int ACC_reduction_op(expr v) {
  ...
  if (strcmp("max", s) == 0)
    return (int)ACC_CLAUSE_REDUCTION_MAX;
  if (strcmp("min", s) == 0)
    return (int)ACC_CLAUSE_REDUCTION_MIN;
  if (strcmp("iand", s) == 0)
    return (int)ACC_CLAUSE_REDUCTION_BITAND;
  if (strcmp("ior", s) == 0)
    return (int)ACC_CLAUSE_REDUCTION_BITOR;
  if (strcmp("ieor", s) == 0)
    return (int)ACC_CLAUSE_REDUCTION_BITXOR;
...
void ACC_check_num_attr(expr v, enum ACC_pragma attr) {
  ...
  a = ACC_CLAUSE_END;
  if (strcmp("num", s) == 0)
    a = ACC_CLAUSE_NUM_WORKERS;
  else if (strcmp("length", s) == 0)
    a = ACC_CLAUSE_VECTOR_LENGTH;
...
static void pop_ACC_construct(enum ACC_pragma dir) {
  if (CTL_TYPE(ctl_top) == CTL_ACC && CTL_ACC_ARG_DIR(ctl_top) == dir) {
    CTL_BLOCK(ctl_top) =
        ACC_pragma_list(dir, CTL_ACC_ARG_CLAUSE(ctl_top), CURRENT_STATEMENTS);
...
static void pop_ACC_loop_construct(enum ACC_pragma dir) {
  ...
  if (EXPR_CODE(statements) != F_DO_STATEMENT) {
    fatal("ACC LOOP | PARALLEL LOOP | KERNELS LOOP directive must be followed "
          "by do statements");
  }
...
expv ACC_pragma_list(enum ACC_pragma pragma, expv arg1, expv arg2) {
  return list3(ACC_PRAGMA, expv_int_term(INT_CONSTANT, NULL, (int)pragma), arg1,
               arg2);
}
