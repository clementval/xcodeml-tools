...
void type_dump(TYPE_DESC tp) { type_output(tp, stderr); }
...
void print_type(TYPE_DESC tp, FILE *fp, int recursive) {
    ...
    if (IS_STRUCT_TYPE(tp) && TYPE_REF(tp) == NULL) {
        ...
        if (TYPE_DIM_LOWER(tp) != NULL)
            expv_dump0(TYPE_DIM_LOWER(tp), fp);
        else
            fprintf(fp, "<NULL>");
        fprintf(fp, ",ub=");
        if (TYPE_DIM_UPPER(tp) != NULL)
            expv_dump0(TYPE_DIM_UPPER(tp), fp);
        else
            fprintf(fp, "<NULL>");
        fprintf(fp, ":");
...
FILE *fp;
{
    ...
    fprintf(fp, ":%d", (EXPR_LINE(x) != NULL) ? EXPR_LINE_NO(x) : 0);
...
void print_controls(fp) FILE *fp;
{
    ...
    for (i = 0; i < CURRENT_BLK_LEVEL; ++i) {
        fprintf(fp, "  [%d] : %s\n", i, control_type_name(ctl->ctltype));
        ctl = CTL_NEXT(ctl);
    }
}
