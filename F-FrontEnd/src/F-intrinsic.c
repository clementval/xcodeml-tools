...
static void generate_reverse_dimension_expr(TYPE_DESC tp, expr dimSpec);
static TYPE_DESC get_intrinsic_return_type(intrinsic_entry *ep, expv args,
                                           expv kindV);
...
int is_intrinsic_function(ID id) {
    return (SYM_TYPE(ID_SYM(id)) == S_INTR) ? TRUE : FALSE;
}

expv compile_intrinsic_call(ID id, expv args) {
    return compile_intrinsic_call0(id, args, FALSE);
}

expv compile_intrinsic_call0(ID id, expv args, int ignoreTypeMismatch) {
    ...
    if (SYM_TYPE(ID_SYM(id)) != S_INTR) {
        ...
        if (tp == NULL) {
            warning_at_node(args, "unknown type of '%s' declared as intrinsic",
                            SYM_NAME(ID_SYM(id)));
    ...
    FOR_ITEMS_IN_LIST(lp, args) { nArgs++; }

    /* Search an intrinsic by checking argument types. */
    found = 0;
    for (; ((INTR_OP(ep) == iOps) && ((strcasecmp(iName, INTR_NAME(ep)) == 0) ||
        ...
        for (i = 0; i < nArgs; i++) {
            ...
            if (i == 1 && INTR_ARG_TYPE(ep)[i] == INTR_TYPE_PASSIGNABLE) {
                if (expv_is_pointer_assignable(NULL, expr_list_get_n(args, 0),
                                               a)) {
                    break;
                }
            }

            if (EXPV_KWOPT_NAME(a) && nIntrArgs > 1 &&
                INTR_ARG_NAME(ep)) { // argument keyword

                int j;
                for (j = 0; j < INTR_N_ARGS(ep); j++) {
                    if (INTR_ARG_NAME(ep)[j]) {
                        if (strcmp(INTR_ARG_NAME(ep)[j], EXPV_KWOPT_NAME(a)) ==
                            0) {
                ...
                if (j == INTR_N_ARGS(ep))
                    typeNotMatch = 1; // not matched
            } else {
                if (compare_intrinsic_arg_type(
                        a, tp,
                        ((isVarArgs == 0) ? INTR_ARG_TYPE(ep)[i]
                                          : INTR_ARG_TYPE(ep)[0])) == 0) {
                    // matched
                    mandatoryArgsFlag &= ~(1 << i);
                    if (isVarArgs == 0 && INTR_ARG_NAME(ep) &&
                        INTR_ARG_NAME(ep)[i] &&
    ...
    if (found == 1) {
        ...
        if (sp == NULL) {
            fatal("%s: symbol '%s' is not created??", __func__, INTR_NAME(ep));
            /* not reached */
            return NULL;
        }
        ...
        ret =
            expv_cons(FUNCTION_CALL, FUNCTION_TYPE_RETURN_TYPE(tp), symV, args);
    }

    if (ret == NULL && !ignoreTypeMismatch) {
        error_at_node((expr)args,
                      "argument(s) mismatch for an intrinsic '%s()'.", iName);
    }
    return ret;
}
...
static int compare_intrinsic_arg_type(expv arg, TYPE_DESC tp,
    ...
    if (isArray == 1) {
        ...
        case INTR_TYPE_INT_ARRAY: {
            if (bType == TYPE_INT || bType == TYPE_GNUMERIC ||
        ...
        case INTR_TYPE_REAL_ARRAY: {
            if (bType == TYPE_REAL || bType == TYPE_GNUMERIC ||
                bType == TYPE_GNUMERIC_ALL) {
                ret = 0;
            }
            break;
        ...
        case INTR_TYPE_DREAL_ARRAY: {
            if (type_is_possible_dreal(tp) || bType == TYPE_GNUMERIC ||
                bType == TYPE_GNUMERIC_ALL) {
                ret = 0;
            }
            break;
        ...
        case INTR_TYPE_ALL_REAL_ARRAY: {
            if (bType == TYPE_REAL || bType == TYPE_DREAL ||
                bType == TYPE_GNUMERIC || bType == TYPE_GNUMERIC_ALL) {
                ret = 0;
            }
            break;
        }
        ...
        case INTR_TYPE_COMPLEX_ARRAY: {
            if (bType == TYPE_COMPLEX || bType == TYPE_DCOMPLEX ||
                bType == TYPE_GNUMERIC_ALL) {
                ret = 0;
            }
            break;
        ...
        case INTR_TYPE_NUMERICS_ARRAY: {
            if (bType == TYPE_INT || bType == TYPE_REAL ||
                bType == TYPE_DREAL || bType == TYPE_GNUMERIC ||
                bType == TYPE_GNUMERIC_ALL) {
                ret = 0;
            }
            break;
        ...
        case INTR_TYPE_ALL_NUMERICS_ARRAY: {
            if (bType == TYPE_INT || bType == TYPE_REAL ||
                bType == TYPE_DREAL || bType == TYPE_GNUMERIC ||
                bType == TYPE_COMPLEX || bType == TYPE_DCOMPLEX ||
                bType == TYPE_GNUMERIC_ALL) {
                ret = 0;
            }
            break;
    ...
    DoCompareBasic:
        ...
        case INTR_TYPE_DCOMPLEX: {
            if (bType == TYPE_DCOMPLEX || bType == TYPE_GNUMERIC ||
                bType == TYPE_GNUMERIC_ALL) {
                ret = 0;
            }
            break;
        ...
        case INTR_TYPE_COARRAY_LOGICAL: {
            if (!isCoarray)
                break;
            return compare_intrinsic_arg_type(arg, tp,
                                              CONVERT_COARRAY_TO_BASIC(iType));
        }
...
void generate_shape_expr(TYPE_DESC tp, expr dimSpec) {
    expv dimElm;

    if ((TYPE_REF(tp) == NULL) || !IS_ARRAY_TYPE(tp))
        return;

    dimElm = list3(F_INDEX_RANGE, TYPE_DIM_LOWER(tp), TYPE_DIM_UPPER(tp),
                   TYPE_DIM_STEP(tp));
...
void generate_assumed_shape_expr(expr dimSpec, int dim) {
    expv dimElm;
...
static void generate_contracted_shape_expr(TYPE_DESC tp, expr dimSpec,
                                           int dim) {
    expv dimElm;
...
static void generate_expand_shape_expr(TYPE_DESC tp, expr dimSpec, expr extDim,
                                       int dim) {
    expv dimElm;
    ...
    //    dimElm = list3(LIST, TYPE_DIM_LOWER(tp), TYPE_DIM_UPPER(tp),
    //    TYPE_DIM_STEP(tp));
...
static TYPE_DESC intr_convert_to_dimension_ifneeded(intrinsic_entry *ep,
                                                    expv args,
                                                    TYPE_DESC ret_tp) {
    TYPE_DESC tp0;
    ...
    if (INTR_IS_ARG_TYPE0_ARRAY(ep) == FALSE && IS_ARRAY_TYPE(tp0)) {
...
static TYPE_DESC get_intrinsic_return_type(intrinsic_entry *ep, expv args,
    ...
    if (INTR_RETURN_TYPE_SAME_AS(ep) >= 0) {
            ...
            if (!(INTR_IS_RETURN_TYPE_DYNAMIC(ep)) &&
                ...
                case INTR_COUNT: {
                    ...
                    if (EXPV_CODE(dim) == INT_CONSTANT) {
                        ...
                        generate_contracted_shape_expr(tp, shape,
                                                       TYPE_N_DIM(tp) - nDim);
                    } else {
                ...
                case INTR_SPREAD: {
                    ...
                    if (EXPR_CODE(dim) == INT_CONSTANT) {
                        ...
                        generate_expand_shape_expr(tp, shape, ncopies,
                                                   TYPE_N_DIM(tp) + 1 - nDim);
                    } else {
                        generate_assumed_shape_expr(shape, TYPE_N_DIM(tp) - 1);
                    }
                } break;
                ...
                case INTR_MATMUL: {
                    ...
                    if (TYPE_N_DIM(t1) == 2 && TYPE_N_DIM(t2) == 2) {
                        /*
                         * (n, m) * (m, k) => (n, k).
                         */
                        shape = list2(LIST, EXPR_ARG1(s1), EXPR_ARG2(s2));
                    } else if (TYPE_N_DIM(t1) == 2 && TYPE_N_DIM(t2) == 1) {
                        /*
                         * (n, m) * (m) => (n).
                         */
                        shape = list1(LIST, EXPR_ARG1(s1));
                    } else if (TYPE_N_DIM(t1) == 1 && TYPE_N_DIM(t2) == 2) {
                        /*
                         * (m) * (m, k) => (k).
                         */
                ...
                case INTR_DOT_PRODUCT: {
                    ...
                    if (TYPE_N_DIM(t1) == 1 && TYPE_N_DIM(t2) == 1) {
                        TYPE_DESC tp =
                            get_binary_numeric_intrinsic_operation_type(t1, t2);
                ...
                case INTR_LCOBOUND: {
                    ...
                    dims = list1(LIST, list2(LIST, make_int_enode(1),
                ...
                case INTR_MINLOC: {
                    ...
                    for (i = TYPE_N_DIM(tp); IS_ARRAY_TYPE(tp);
                         i--, tp = TYPE_REF(tp)) {
                        TYPE_DESC tp0;
                        ...
                        TYPE_ARRAY_ASSUME_KIND(tp0) =
                            TYPE_ARRAY_ASSUME_KIND(tp);
        ...
        case -3: {
            ...
            if (INTR_OP(ep) == INTR_MAXLOC || INTR_OP(ep) == INTR_MINLOC) {
        ...
        case -5: {
            ...
            case INTR_DIMAG: {
                ...
                bType = (bType == TYPE_COMPLEX) ? TYPE_REAL : TYPE_DREAL;
                break;
            }
            default: {
                bType = intr_type_to_basic_type(INTR_RETURN_TYPE(ep));
