...
expv compile_intrinsic_call0(ID id, expv args, int ignoreTypeMismatch) {
        ...
        for (j = 0; j < INTR_N_ARGS(ep); j++) {
          if (INTR_ARG_NAME(ep)[j]) {
            if (strcmp(INTR_ARG_NAME(ep)[j], EXPV_KWOPT_NAME(a)) == 0) {
              ...
              if (compare_intrinsic_arg_type(a, tp, INTR_ARG_TYPE(ep)[j]) ==
                  0) {
  ...
  if (ret == NULL && !ignoreTypeMismatch) {
    error_at_node((expr)args, "argument(s) mismatch for an intrinsic '%s()'.",
                  iName);
  }
  return ret;
}
...
static int compare_intrinsic_arg_type(expv arg, TYPE_DESC tp,
  ...
  if (isArray == 1) {
    ...
    case INTR_TYPE_ALL_REAL_ARRAY: {
      if (bType == TYPE_REAL || bType == TYPE_DREAL || bType == TYPE_GNUMERIC ||
          bType == TYPE_GNUMERIC_ALL) {
        ret = 0;
      }
      break;
    ...
    case INTR_TYPE_NUMERICS_ARRAY: {
      if (bType == TYPE_INT || bType == TYPE_REAL || bType == TYPE_DREAL ||
    ...
    case INTR_TYPE_ALL_NUMERICS_ARRAY: {
      if (bType == TYPE_INT || bType == TYPE_REAL || bType == TYPE_DREAL ||
          bType == TYPE_GNUMERIC || bType == TYPE_COMPLEX ||
          bType == TYPE_DCOMPLEX || bType == TYPE_GNUMERIC_ALL) {
        ret = 0;
      }
      break;
    }
    ...
    case -6 /* if not dynamic return type,
      ...
      if (!(INTR_IS_RETURN_TYPE_DYNAMIC(ep)) &&
        ...
        if (bType == TYPE_UNKNOWN) {
          ...
          if (kindV == NULL) {
            ret = (bType != TYPE_CHAR) ? type_basic(bType) : type_char(1);
          } else {
            /*
        ...
        case INTR_DOT_PRODUCT: {
          ...
          if (TYPE_N_DIM(t1) == 1 && TYPE_N_DIM(t2) == 1) {
            TYPE_DESC tp = get_binary_numeric_intrinsic_operation_type(t1, t2);
        ...
        case INTR_LCOBOUND: {
          ...
          dims = list1(LIST,
                       list2(LIST, make_int_enode(1), make_int_enode(corank)));
