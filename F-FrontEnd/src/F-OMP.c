...
void compile_OMP_pragma_clause(expr x, int pragma, int is_parallel, expv *pc,
                               expv *dc);
...
enum OMP_st_pragma { OMP_ST_NONE, OMP_ST_ATOMIC, OMP_ST_END };
...
int OMP_reduction_op(expr v) {
  ...
  if (strcmp("max", s) == 0)
    return (int)OMP_DATA_REDUCTION_MAX;
  if (strcmp("min", s) == 0)
    return (int)OMP_DATA_REDUCTION_MIN;
  if (strcmp("iand", s) == 0)
    return (int)OMP_DATA_REDUCTION_BITAND;
  if (strcmp("ior", s) == 0)
    return (int)OMP_DATA_REDUCTION_BITOR;
  if (strcmp("ieor", s) == 0)
    return (int)OMP_DATA_REDUCTION_BITXOR;
...
int OMP_depend_op(expv v) {
  ...
  s = SYM_NAME(EXPR_SYM(v));
  if (strcmp("in", s) == 0)
    return (int)OMP_DATA_DEPEND_IN;
  if (strcmp("out", s) == 0)
    return (int)OMP_DATA_DEPEND_OUT;
  if (strcmp("inout", s) == 0)
    return (int)OMP_DATA_DEPEND_INOUT;
...
void check_for_OMP_pragma(expr x) {
  if (OMP_do_required) {
    if (EXPR_CODE(x) == XMP_PRAGMA && EXPR_INT(EXPR_ARG1(x)) == XMP_LOOP)
      return;
    if (is_ACC_loop_pragma(x))
      return;
  ...
  if (CTL_TYPE(ctl_top) == CTL_OMP &&
    ...
    if (CTL_OMP_ARG_DIR(ctl_top) == OMP_F_PARALLEL_DO) {
      ...
      CTL_BLOCK(ctl_top) =
          OMP_FOR_pragma_list(CTL_OMP_ARG_DCLAUSE(ctl_top), CURRENT_STATEMENTS);
    }
...
int OMP_output_st_pragma(expv v) {
  ...
  case OMP_ST_ATOMIC:
    output_statement(
        OMP_pragma_list(OMP_ATOMIC, list0(LIST), list1(EXPR_STATEMENT, v)));
    return TRUE;
  default:
    return FALSE;
  }
...
expv OMP_pragma_list(enum OMP_pragma pragma, expv arg1, expv arg2) {
  return list3(OMP_PRAGMA, expv_int_term(INT_CONSTANT, NULL, (int)pragma), arg1,
               arg2);
}
...
expv OMP_FOR_pragma_list(expv clause, expv statements) {
  ...
  FOR_ITEMS_IN_LIST(lp, statements) {
    ...
    if (EXPR_CODE(xx) == F_DO_STATEMENT || is_ACC_loop_pragma(xx)) {
...
void compile_OMP_directive(expr x) {
  ...
  if (EXPR_INT(dir) != OMP_F_END_DO && EXPR_INT(dir) != OMP_F_END_PARALLEL_DO &&
  ...
  case OMP_F_PARALLEL:
    push_ctl(CTL_OMP);
    compile_OMP_pragma_clause(EXPR_ARG2(x), OMP_PARALLEL, TRUE, &pclause,
                              &dclause);
  ...
  case OMP_F_END_PARALLEL:
    if (CTL_TYPE(ctl_top) == CTL_OMP &&
      ...
      CTL_BLOCK(ctl_top) = OMP_pragma_list(
          OMP_PARALLEL, CTL_OMP_ARG_PCLAUSE(ctl_top), CURRENT_STATEMENTS);
      EXPR_LINE(CTL_BLOCK(ctl_top)) = EXPR_LINE(CTL_OMP_ARG(ctl_top));
      pop_ctl();
    } else
      error("OpenMP PARALLEL block is not closed");
    return;

  case OMP_F_PARALLEL_DO:
    push_ctl(CTL_OMP);
    compile_OMP_pragma_clause(EXPR_ARG2(x), OMP_FOR, TRUE, &pclause, &dclause);
    CTL_OMP_ARG(ctl_top) = list3(LIST, dir, pclause, dclause);
    EXPR_LINE(CTL_OMP_ARG(ctl_top)) = current_line;
  ...
  case OMP_F_DO:
    push_ctl(CTL_OMP);
    compile_OMP_pragma_clause(EXPR_ARG2(x), OMP_FOR, FALSE, &pclause, &dclause);
    CTL_OMP_ARG(ctl_top) = list3(LIST, dir, pclause, dclause);
    EXPR_LINE(CTL_OMP_ARG(ctl_top)) = current_line;
    OMP_do_required = TRUE;
    return;
  case OMP_F_END_DO:
    ...
    if (ctl_top_saved) {
      ...
      if (EXPR_ARG2(x) != NULL)
        list_put_last(dclause, EXPR_ARG2(x));
      CTL_BLOCK(ctl_top_saved) =
          OMP_FOR_pragma_list(dclause, CURRENT_STATEMENTS_saved);
      EXPR_LINE(CTL_BLOCK(ctl_top_saved)) =
          EXPR_LINE(CTL_OMP_ARG(ctl_top_saved));
      ctl_top_saved = NULL;
    }
  ...
  case OMP_F_PARALLEL_SECTIONS:
    push_ctl(CTL_OMP);
    compile_OMP_pragma_clause(EXPR_ARG2(x), OMP_SECTIONS, TRUE, &pclause,
                              &dclause);
    CTL_OMP_ARG(ctl_top) = list3(LIST, dir, pclause, dclause);
    EXPR_LINE(CTL_OMP_ARG(ctl_top)) = current_line;
    return;
  case OMP_F_END_PARALLEL_SECTIONS:
    if (CTL_TYPE(ctl_top) == CTL_OMP &&
      ...
      CTL_BLOCK(ctl_top) = OMP_pragma_list(
          ...
          OMP_pragma_list(OMP_SECTIONS, CTL_OMP_ARG_DCLAUSE(ctl_top),
                          CURRENT_STATEMENTS));
      EXPR_LINE(CTL_BLOCK(ctl_top)) = EXPR_LINE(CTL_OMP_ARG(ctl_top));
      pop_ctl();
    } else
      error("OpenMP PARALLEL SECTIONS block is not closed");
    return;

  case OMP_F_SECTIONS:
    push_ctl(CTL_OMP);
    compile_OMP_pragma_clause(EXPR_ARG2(x), OMP_SECTIONS, FALSE, &pclause,
                              &dclause);
    CTL_OMP_ARG(ctl_top) = list3(LIST, dir, pclause, dclause);
    EXPR_LINE(CTL_OMP_ARG(ctl_top)) = current_line;
    return;
  case OMP_F_END_SECTIONS:
    if (CTL_TYPE(ctl_top) == CTL_OMP &&
      ...
      error("OpenMP SECTIONS block is not closed");
    return;
  ...
  case OMP_F_SECTION:
    if (CTL_TYPE(ctl_top) == CTL_OMP &&
      ...
      error("OpenMP SECTION appears outside SECTOINS");
    return;
  ...
  case OMP_F_END_SINGLE:
    if (CTL_TYPE(ctl_top) == CTL_OMP &&
      ...
      error("OpenMP SINGLE block is not closed");
    return;
  ...
  case OMP_F_END_MASTER:
    if (CTL_TYPE(ctl_top) == CTL_OMP &&
      ...
      error("OpenMP MASTER block is not closed");
    return;
  ...
  case OMP_F_END_ORDERED:
    if (CTL_TYPE(ctl_top) == CTL_OMP &&
      ...
      error("OpenMP ORDERED block is not closed");
    return;
  ...
  case OMP_F_END_CRITICAL:
    if (CTL_TYPE(ctl_top) == CTL_OMP &&
        ...
        ((EXPR_ARG2(x) == NULL && EXPR_ARG2(CTL_OMP_ARG(ctl_top)) == NULL) ||
         (EXPR_ARG2(x) != NULL && EXPR_ARG2(CTL_OMP_ARG(ctl_top)) != NULL &&
          EXPR_SYM(EXPR_ARG1(EXPR_ARG2(x))) ==
              EXPR_SYM(EXPR_ARG1(EXPR_ARG2(CTL_OMP_ARG(ctl_top))))))) {
      CTL_BLOCK(ctl_top) = OMP_pragma_list(
          OMP_CRITICAL, EXPR_ARG2(CTL_OMP_ARG(ctl_top)), CURRENT_STATEMENTS);
      EXPR_LINE(CTL_BLOCK(ctl_top)) = EXPR_LINE(CTL_OMP_ARG(ctl_top));
      pop_ctl();
    } else
      error("OpenMP CRITICAL block is not closed");
    return;
  ...
  case OMP_F_END_TASK:
    if (CTL_TYPE(ctl_top) == CTL_OMP &&
      ...
      CTL_BLOCK(ctl_top) = OMP_pragma_list(
          OMP_TASK, CTL_OMP_ARG_DCLAUSE(ctl_top), CURRENT_STATEMENTS);
      EXPR_LINE(CTL_BLOCK(ctl_top)) = EXPR_LINE(CTL_OMP_ARG(ctl_top));
      pop_ctl();
    } else
      error("OpenMP TASK block is not closed");
    return;

  case OMP_F_SIMD:
    push_ctl(CTL_OMP);
    compile_OMP_pragma_clause(EXPR_ARG2(x), OMP_SIMD, TRUE, &pclause, &dclause);
    CTL_OMP_ARG(ctl_top) = list3(LIST, dir, pclause, dclause);
    EXPR_LINE(CTL_OMP_ARG(ctl_top)) = current_line;
    return;
  ...
  case OMP_F_END_SIMD:
    if (CTL_TYPE(ctl_top) == CTL_OMP &&
      ...
      CTL_BLOCK(ctl_top) = OMP_pragma_list(
          OMP_SIMD, CTL_OMP_ARG_DCLAUSE(ctl_top), CURRENT_STATEMENTS);
      EXPR_LINE(CTL_BLOCK(ctl_top)) = EXPR_LINE(CTL_OMP_ARG(ctl_top));
      pop_ctl();
    } else
      error("OpenMP SIMD block is not closed");
    return;
  ...
  case OMP_F_END_DO_SIMD:
    if (CTL_TYPE(ctl_top) == CTL_OMP &&
      ...
      CTL_BLOCK(ctl_top) = OMP_pragma_list(
          OMP_FOR, CTL_OMP_ARG_DCLAUSE(ctl_top),
      ...
      error("OpenMP DO SIMD block is not closed");
    return;
  ...
  case OMP_F_END_DECLARE_SIMD:
    if (CTL_TYPE(ctl_top) == CTL_OMP &&
      ...
      CTL_BLOCK(ctl_top) = OMP_pragma_list(
          OMP_DECLARE, CTL_OMP_ARG_DCLAUSE(ctl_top),
          OMP_pragma_list(OMP_SIMD, CTL_OMP_ARG_DCLAUSE(ctl_top),
                          CURRENT_STATEMENTS));
      ...
      error("OpenMP DECLARE SIMD block is not closed");
    return;
  ...
  case OMP_F_END_PARALLEL_DO_SIMD:
    if (CTL_TYPE(ctl_top) == CTL_OMP &&
      ...
      CTL_BLOCK(ctl_top) = OMP_pragma_list(
          OMP_PARALLEL, CTL_OMP_ARG_DCLAUSE(ctl_top),
      ...
      error("OpenMP PARALLEL DO SIMD block is not closed");
    return;
...
expv OMP_check_SECTION(expr x) {
  ...
  if (x == NULL)
    return NULL;
  if (EXPR_CODE(x) != LIST)
    fatal("OMP_check_SECION: not LIST");

  section_list = EMPTY_LIST;
  current_section = NULL;
  FOR_ITEMS_IN_LIST(lp, x) {
    xx = LIST_ITEM(lp);
    if (EXPR_CODE(xx) == OMP_PRAGMA && EXPR_INT(EXPR_ARG1(xx)) == OMP_SECTION) {
...
void compile_OMP_pragma_clause(expr x, int pragma, int is_parallel, expv *pc,
  ...
  FOR_ITEMS_IN_LIST(lp, x) {
    ...
    case OMP_DATA_DEFAULT: /* default(shared|none|private) */
      ...
      pclause = list_put_last(
          pclause, list2(LIST, EXPR_ARG1(c), EXPR_ARG1(EXPR_ARG2(c))));
      break;
    ...
    case OMP_DIR_SCHEDULE:
      ...
      if (v != NULL) {
        ...
        c = list2(LIST, EXPR_ARG1(c), list2(LIST, EXPR_ARG1(EXPR_ARG2(c)), v));
      }
      dclause = list_put_last(dclause, c);
      break;
...
ret:
  ...
  if (is_parallel && (pragma != OMP_PARALLEL))
    dclause = list_put_last(
        dclause,
        list2(LIST, make_enode(INT_CONSTANT, ((void *)OMP_DIR_NOWAIT)), NULL));
  *pc = pclause;
  *dc = dclause;
}
...
struct {
  ...
  BASIC_DATA_TYPE type;
    ...
    {"omp_get_max_threads", TYPE_INT},  {"omp_get_thread_num", TYPE_INT},
    {"omp_get_num_procs", TYPE_INT},    {"omp_in_parallel", TYPE_LOGICAL},
    {"omp_set_dynamic", TYPE_SUBR},     {"omp_get_dynmaic", TYPE_LOGICAL},
    {"omp_set_nested", TYPE_SUBR},      {"omp_get_nested", TYPE_LOGICAL},
    {"omp_init_lock", TYPE_SUBR},       {"omp_destroy_lock", TYPE_SUBR},
    {"omp_set_lock", TYPE_SUBR},        {"omp_unset_lock", TYPE_SUBR},
    {"omp_test_lock", TYPE_LOGICAL},    {NULL}};
...
found:
  if (cdir == OMP_DATA_PRIVATE || cdir == OMP_DATA_FIRSTPRIVATE ||
