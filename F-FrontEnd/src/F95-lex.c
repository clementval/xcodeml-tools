...
static int read_number() {
  ...
  *p = '\0';
  ...
  if (have_dot || have_exp) {
    ...
    yylval.val =
        make_float_enode(exp_code, convert_str_double(buffio), strdup(buffio));
  } else {
    omllint_t v = 0;
...
again:
  ...
  if (isdigit(*p)) {
    ...
    if (is_pragma_sentinel(&sentinels, p, &index)) {
      ...
      if (strcasecmp(sentinel_name(&sentinels, index), OMP_SENTINEL) == 0) {
        ...
        set_pragma_str(buff);
        st_PRAGMA_flag = TRUE;
      } else if (strcasecmp(sentinel_name(&sentinels, index), CDIR_SENTINEL) ==
                 0) {
        char buff[256] = "cdir";
        strcat(buff, p);
        set_pragma_str(buff);
  ...
  next_line:
    ...
    if (!OMP_flag && !XMP_flag && !ACC_flag && !cond_compile_enabled &&
      ...
      q = st_buffer + st_len;
      l = ScanFortranLine(pp, oBuf, q, st_buffer, bufMax, &inQuote, &qChar,
                          &inH, &hLen, &p, &q);
      st_len += l;
      if (st_len >= ST_BUF_SIZE) {
        goto Done;
...
copy_body:
  ...
  copy_body_cont:
    ...
    if ((!OMP_flag && !XMP_flag && !ACC_flag && !cond_compile_enabled) &&
      ...
      /* oBuf => st_buffer */
      newLen = ScanFortranLine(p, oBuf, q, st_buffer, bufMax, &inQuote, &qChar,
                               &inH, &hLen, &p, &q);
      st_len += newLen;
    }
    if (st_len >= ST_BUF_SIZE) {
...
KeepOnGoin:
  ...
  if (line_buffer[body_offset] == '\n') {
    ...
    for (i = 0; i < sizeof(scanBuf) && (i + body_offset) < LINE_BUF_SIZE; i++) {
      ...
      if (c == '!' && inQuote == 0) {
        ...
        if (!OMP_flag &&
            (is_cond_compilation(&sentinels, stn_cols))) { /* NEED FIX */
          ...
          no_countup = TRUE;
          if (fseek(source_file, starting_pos + body_offset + i, SEEK_SET) !=
              0) {
            error("fseek error.");
          }
          goto Newline;
  ...
  if (IS_CONT_LINE(stn_cols)) {
    if ((st_OMP_flag && !local_OMP_flag) || (!st_OMP_flag && local_OMP_flag)) {
      error("bad OMP sentinel continuation line");
      return (ST_INIT);
    }
    if ((st_XMP_flag && !local_XMP_flag) || (!st_XMP_flag && local_XMP_flag)) {
      error("bad XMP sentinel continuation line");
      return (ST_INIT);
    }
    if ((st_ACC_flag && !local_ACC_flag) || (!st_ACC_flag && local_ACC_flag)) {
      error("bad ACC sentinel continuation line");
      return (ST_INIT);
    }
...
static int is_fixed_cond_statement_label(char *label) {
  if (strlen(label) < 6)
    return FALSE;
  if (label[0] != '!' && label[0] != '*' && label[0] != 'c' && label[0] != 'C')
    return FALSE;
  if (label[1] != '$')
    return FALSE;
...
static void _warning_if_doubtfulLongLine(char *buf, int maxLen) {
  ...
  warning_lineno(&read_lineno, "line contains more than %d characters", maxLen);
}
    ...
    *bp = '\0';
    ...
    for (i = 0; unHToken[i].key != NULL; i++) {
      ...
      if (kStart < buf) {
        ...
        if (strncasecmp(kStart, unHToken[i].key, unHToken[i].len) == 0) {
          found++;
          break;
        }
      }
      ...
      if (*inHollerithPtr == FALSE && *inQuotePtr == FALSE) {
        ...
        if (isalpha((int)*cur) || *cur == '_') {
          ...
          unHollerith(src, srcHead, dst, dstHead, dstMax, inQuotePtr,
                      *quoteCharPtr, inHollerithPtr, hollerithLenPtr, &src,
                      &dst);
        }
      } else {
        goto copyOne;
...
/* TOKEN DATA */
struct keyword_token dot_keywords[] = {{"and.", AND},
                                       {"or.", OR},
                                       {"not.", NOT},
                                       {"true.", TRUE_CONSTANT},
