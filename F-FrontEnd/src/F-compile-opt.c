...
TN *cons_expr(opcode, lp, rp) enum tree_opcode opcode;
TN *lp, *rp;
{
  TN *e, *e1;
  enum datatype etype;
...
TN *make_power(p) TN *p;
{
  ...
  if (IS_INT(rtype)) {
    if (ltype == TYSHORT && rtype == TYSHORT &&
      ...
      q = call2(ltype, powint[(int)ltype - (int)TYLONG], lp,
                make_conv(TYLONG, rp));
    }
  } else if (IS_REAL((mtype = maxtype(ltype, rtype))))
    q = call2(mtype, "pow_dd", make_conv(TYDREAL, lp), make_conv(TYDREAL, rp));
  else {
    q = call2(TYDCOMPLEX, "pow_zz", make_conv(TYDCOMPLEX, lp),
              make_conv(TYDCOMPLEX, rp));
    if (mtype == TYCOMPLEX)
