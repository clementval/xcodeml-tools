...
static int varComp(const void *v1, const void *v2) {
  return strcmp(((variableEntry *)v1)->varName, ((variableEntry *)v2)->varName);
}
...
omllint_t getExprValue(expv v) {
  ...
  default: {
    error(
        "only integer expression is allowed in implied DO in DATA statement.");
    return 0;
  }
  }
    ...
    FOR_ITEMS_IN_LIST(lp, EXPR_ARG2(doSpec)) {
      ...
      case F_ARRAY_REF: {
        ...
        FOR_ITEMS_IN_LIST(lq, EXPR_ARG2(v)) {
          ...
          idxV =
              list_put_last(idxV, expv_int_term(INT_CONSTANT, type_INT, idx));
        }
