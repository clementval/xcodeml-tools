...
expv compile_terminal_node(x) expr x;
{
  ...
  case FLOAT_CONSTANT: {
    TYPE_DESC tp = type_basic((EXPR_CODE(x) == F_DOUBLE_CONSTANT) ? TYPE_DREAL
                                                                  : TYPE_REAL);
    ...
    if (ret != NULL && EXPR_CODE(x) == F_QUAD_CONSTANT) {
       ...
       * specifier _16.
       */
      TYPE_KIND(EXPV_TYPE(ret)) = expv_int_term(INT_CONSTANT, type_INT, 16);
    }
    break;
  }
    ...
    for (i = 0; i < nDims; i++) {
      ...
      if (laSz > 0 && raSz > 0) {
        if (!for_argument && laSz == raSz) {
          ...
          /*
           * The actual argument can be longer then the dummy argument. Use
           * left.
           */
          aSpec = laSpec;
        } else {
...
expv compile_expression(expr x) {
  ...
  case F_ARRAY_REF: { /* (F_ARRAY_REF name args) */
    ...
    if ((ID_CLASS(id) == CL_PROC || ID_CLASS(id) == CL_ENTRY ||
      ...
      // In the following code snippet, although f is a name of the function,
      // the reference to it in line 3 isn't a recursive call to it but an
      // reference to its array-valued result value.
      //
      // function f()
      // real f(8)
      // a = f(1)
  ...
  case F95_KIND_SELECTOR_SPEC: {
    ...
    if (v != NULL && !expv_is_specification(v)) {
      error_at_node(EXPR_ARG1(x), "kind must be a specification expression.");
    }
    if (v != NULL) {
  ...
  case F_STRING_CONST_SUBSTR: {
    ...
    typeDeclX = list3(
        ...
        (is_in_module() == FALSE)
            ? list1(LIST, list0(F95_PARAMETER_SPEC))
            : list2(LIST, list0(F95_PARAMETER_SPEC), list0(F95_PRIVATE_SPEC)));
...
static expv compile_logical_expression0(expr x, int allowArray) {
  ...
  } else if (!IS_LOGICAL(tp) && !IS_GENERIC_TYPE(tp) && !IS_GNUMERIC_ALL(tp)) {
    /*
     * FIXME
...
static int checkSubscriptIsInt(expv v) {
  ...
  if (IS_ARRAY_TYPE(tp) && IS_INT(array_element_type(tp)) == FALSE) {
    error_at_node(v, "subscript must be integer or integer array expression");
    return FALSE;
  }
...
static int compile_array_ref_dimension(expr args, expv subs, expv aSpecs) {
  ...
  FOR_ITEMS_IN_LIST(lp, args) {
    ...
    if (EXPR_CODE(v) == F_INDEX_RANGE) {
      ...
      error_at_node(d, "#%d subscrition is not an integer expression.", n + 1);
      continue;
    }
...
expv compile_coarray_ref(expr coarrayRef) {
  ...
  if (obj) {
    ...
    if (EXPV_CODE(obj2) == F95_MEMBER_REF) {
      ...
      if (!check_ancestor(parent)) {
        error_at_node(coarrayRef, "Each ancestor of the coarray component must "
...
expv compile_function_call_check_intrinsic_arg_type(ID f_id, expr args,
  ...
  if (ID_CLASS(f_id) == CL_VAR && IS_PROCEDURE_TYPE(ID_TYPE(f_id))) {
    ...
    a = compile_args(args);
    v = list3(FUNCTION_CALL, expv_sym_term(F_VAR, ID_TYPE(f_id), ID_SYM(f_id)),
              a, expv_any_term(F_EXTFUNC, f_id));
  ...
  case P_UNDEFINEDPROC:
    ...
    v = list3(FUNCTION_CALL, ID_ADDR(f_id), a, expv_any_term(F_EXTFUNC, f_id));
  ...
  FOR_ITEMS_IN_LIST(lp, type_param_args) {
    ...
    if (EXPR_CODE(arg) == F_SET_EXPR) {
      ...
      if (has_keyword == TRUE) {
        ...
        error("KEYWORD connot be ommited after the type parameter value with a "
              "keyword");
        return FALSE;
      }
    ...
    default:
      ...
      if (!expr_is_constant_typeof(
              EXPR_CODE(arg) == F_SET_EXPR ? EXPR_ARG2(arg) : arg, TYPE_INT)) {
...
static expv compile_struct_constructor_with_components(const ID struct_id,
  ...
  FOR_ITEMS_IN_LIST(lp, args) {
    ...
    if (TYPE_IS_POINTER(ID_TYPE(match))) {
      ...
      if (!type_is_compatible_for_assignment(ID_TYPE(match), EXPV_TYPE(v))) {
        error("type is not applicable in struct constructor");
        return NULL;
      }
    }
...
expv compile_args(expr args) {
  ...
  FOR_ITEMS_IN_LIST(lp, args) {
    ...
    if (EXPR_CODE(a) == IDENT) {
      ...
      case CL_PROC:
      case CL_ENTRY:
        if (PROC_CLASS(id) != P_THISPROC && (declare_function(id) == NULL)) {
          continue;
        }
        break;
      case CL_VAR:
