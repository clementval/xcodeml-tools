...
expv expv_reduce(expv v, int doParamReduce) {
  ...
  if (v == NULL)
    return (v); /* error recovery */

  code = EXPV_CODE(v);
  if (doParamReduce == TRUE && code == F_VAR) {
    /* Could be a parameter. */
    ID vId = find_ident(EXPV_NAME(v));
    if (vId != NULL && (ID_CLASS(vId) == CL_PARAM || TYPE_IS_PARAMETER(vId))) {
  ...
  if (doParamReduce && code == FUNCTION_CALL &&
    ...
    arg = expv_reduce(expr_list_get_n(expr_list_get_n(v, 1), 0), TRUE);
    if (arg && EXPR_CODE(arg) == INT_CONSTANT) {
      ...
      if (0 <= n && n <= 9)
        ...
        bytes = 16; // 128bit int
      if (bytes > 0)
        return expv_int_term(INT_CONSTANT, type_basic(TYPE_INT), bytes);
    }
  }
  ...
  if (lcode == FUNCTION_CALL || rcode == FUNCTION_CALL ||
      lcode == F95_MEMBER_REF || rcode == F95_MEMBER_REF ||
      lcode == F_ARRAY_REF || rcode == F_ARRAY_REF || lcode == ARRAY_REF ||
      rcode == ARRAY_REF) {
    return v;
  }
  ...
  case PLUS_EXPR:
    ...
    if (rcode == INT_CONSTANT && lcode == PLUS_EXPR &&
      ...
      return (
          expv_cons(code, tp, EXPV_LEFT(left),
  ...
  case UNARY_MINUS_EXPR:
    if (lcode == INT_CONSTANT) {
      ret = expv_int_term(INT_CONSTANT, EXPV_TYPE(v), -EXPV_INT_VALUE(left));
