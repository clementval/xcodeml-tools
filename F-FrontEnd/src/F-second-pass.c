        ...
        if (EXT_SYM(ep)) {
            ...
            if (EXT_PROC_CONT_EXT_SYMS(ep)) {
                FOREACH_ID(id, EXT_PROC_ID_LIST(EXT_PROC_CONT_EXT_SYMS(ep))) {
                    if (id) {
                        printf("  contains symbol name: %s(%p)(class=%d)",
                               ID_NAME(id), ID_SYM(id), ID_CLASS(id));
                        if (ID_TYPE(id) == NULL) {
                            printf(" (type null)");
                        } else {
                            printf(" (type = %d)", ID_TYPE(id)->basic_type);
    ...
    for (sp_list = sp_list_head; sp_list != NULL; sp_list = sp_list->next) {
        ...
        if (sp_list->type == SP_LIST_TYPE_ID) { /* ID */
            ...
            if (EXT_PROC_CONT_EXT_SYMS(sp_list->nest_ext_id[0])) {
                ID id;
                FOREACH_ID(id, EXT_PROC_ID_LIST(EXT_PROC_CONT_EXT_SYMS(
                                   sp_list->nest_ext_id[0]))) {
            ...
            if (is_exist) {
                ...
                FOREACH_ID(id, EXT_PROC_ID_LIST(
                                   sp_list->nest_ext_id[sp_list->nest_level])) {
                    if (id == sp_list->info.id) {
                        if (ID_TYPE(id) && IS_FUNCTION_TYPE(ID_TYPE(id))) {
                             ...
                             *
                             * If removing it,
                             * the expression of the function call losts the
                             * type id from <typeTable/>, then F-BackEnd fails.
                             */
