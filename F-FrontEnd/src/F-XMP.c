...
enum XMP_st_pragma { XMP_ST_NONE, XMP_ST_GMOVE, XMP_ST_ARRAY, XMP_ST_END };
...
int XMP_reduction_op(expr v) {
  ...
  if (strcmp("max", s) == 0)
    return (int)XMP_DATA_REDUCE_MAX;
  if (strcmp("min", s) == 0)
    return (int)XMP_DATA_REDUCE_MIN;
  if (strcmp("iand", s) == 0)
    return (int)XMP_DATA_REDUCE_BAND;
  if (strcmp("ior", s) == 0)
    return (int)XMP_DATA_REDUCE_BOR;
  if (strcmp("ieor", s) == 0)
    return (int)XMP_DATA_REDUCE_BXOR;

  if (strcmp("firstmax", s) == 0)
    return (int)XMP_DATA_REDUCE_FIRSTMAX;
  if (strcmp("firstmin", s) == 0)
    return (int)XMP_DATA_REDUCE_FIRSTMIN;
  if (strcmp("lastmax", s) == 0)
    return (int)XMP_DATA_REDUCE_LASTMAX;
  if (strcmp("lastmin", s) == 0)
    return (int)XMP_DATA_REDUCE_LASTMIN;
...
expv XMP_pragma_list(enum XMP_pragma pragma, expv arg1, expv arg2) {
  return list3(XMP_PRAGMA, expv_int_term(INT_CONSTANT, NULL, (int)pragma), arg1,
               arg2);
}
...
void compile_XMP_directive(expr x) {
  ...
  if (EXPR_INT(dir) != XMP_END_TASKS && EXPR_INT(dir) != XMP_END_TASK)
  ...
  case XMP_END_TASK:
    check_INEXEC();
    if (CTL_TYPE(ctl_top) == CTL_XMP && CTL_XMP_ARG_DIR(ctl_top) == XMP_TASK) {
  ...
  case XMP_END_TASKS:
    check_INEXEC();
    if (CTL_TYPE(ctl_top) == CTL_XMP && CTL_XMP_ARG_DIR(ctl_top) == XMP_TASKS) {
...
static int isIOStatement(expr x) {
  return (EXPR_CODE(x) == F_PRINT_STATEMENT ||
          ...
          EXPR_CODE(x) == F_INQUIRE_STATEMENT)
             ? 1
             : 0;
}
...
int check_for_XMP_pragma(int st_no, expr x) {
  ...
  if (XMP_do_required) {
    ...
    if (EXPR_CODE(x) == F_PRAGMA_STATEMENT)
      goto done;
    if (EXPR_CODE(x) == LIST && EXPR_INT(EXPR_ARG1(x)) == OMP_F_PARALLEL_DO)
      goto done;
  ...
  if (CTL_TYPE(ctl_top) == CTL_XMP && CTL_XMP_ARG_DIR(ctl_top) == XMP_LOOP) {
    ...
    CTL_BLOCK(ctl_top) =
        XMP_pragma_list(XMP_LOOP, CTL_XMP_ARG_CLAUSE(ctl_top), statements);
    EXPR_LINE(CTL_BLOCK(ctl_top)) = EXPR_LINE(CTL_XMP_ARG(ctl_top));
    pop_ctl();
    goto done;
  }
...
expv XMP_check_TASK(expr x) {
  ...
  if (x == NULL)
    return NULL;
  if (EXPR_CODE(x) != LIST)
    fatal("XMP_check_TASK: not LIST");

  task_list = EMPTY_LIST;

  FOR_ITEMS_IN_LIST(lp, x) {
    xx = LIST_ITEM(lp);
    if (EXPR_CODE(xx) == XMP_PRAGMA && EXPR_INT(EXPR_ARG1(xx)) == XMP_TASK) {
...
expv XMP_compile_subscript_list(expr l, xmp_list_context context) {
  ...
  FOR_ITEMS_IN_LIST(lp, l) {
    ...
    case XMP_LIST_DISTRIBUTE: /* * or (id expr) */
      if (x != NULL) {
        if (EXPR_ARG2(x) != NULL) {
          v = list2(LIST, EXPR_ARG1(x), compile_expression(EXPR_ARG2(x)));
        } else
          v = x;
      }
      break;
      ...
      if (x != NULL) {
        if (!EXPR_ARG1(x) && !EXPR_ARG2(x) && !EXPR_ARG3(x)) {
          ...
          break;
                   ...
                   EXPR_ARG3(x) && EXPR_CODE(EXPR_ARG3(x)) == INT_CONSTANT &&
                   EXPR_INT(EXPR_ARG3(x)) == 0) {
          /* expression */
          x = EXPR_ARG1(x);
      ...
      if (x != NULL) {
        if (!EXPR_ARG1(x) && !EXPR_ARG2(x) && !EXPR_ARG3(x)) {
          ...
          case F_MINUS_EXPR:
            ...
            v = expv_cons(EXPR_CODE(x) == F_PLUS_EXPR ? PLUS_EXPR : MINUS_EXPR,
...
static expv XMP_compile_acc_clause(expr x) { return compile_expression(x); }
