  ...
  if (IS_STRUCT_TYPE(tp)) {
    ...
    FOREACH_MEMBER(id, tp) {
      if (TYPE_IS_COINDEXED(ID_TYPE(id)))
        return TRUE;
      if (IS_STRUCT_TYPE(ID_TYPE(id)) && !TYPE_IS_ALLOCATABLE(ID_TYPE(id)) &&
  ...
  if (is_strict == TRUE) {
    ...
    if (EXPR_CODE(v1) != INT_CONSTANT) {
      if (EXPR_CODE(v1) == F08_LEN_SPEC_COLON ||
          EXPR_CODE(v1) == LEN_SPEC_ASTERISC || EXPR_CODE(v1) == F_ASTERISK) {
      ...
      if (EXPR_CODE(v2) == F08_LEN_SPEC_COLON ||
          EXPR_CODE(v2) == LEN_SPEC_ASTERISC || EXPR_CODE(v2) == F_ASTERISK) {
        return FALSE;
      }
    }
  }
  ...
  if (EXPR_CODE(v1) == FUNCTION_CALL && EXPR_CODE(v2) == FUNCTION_CALL) {
    ...
    if (strcmp(name1, name2) == 0) {
      ...
      if (strcmp(name1, "selected_real_kind") == 0) {
        if (!EXPR_HAS_ARG1(EXPR_ARG2(v1)) && !EXPR_HAS_ARG1(EXPR_ARG2(v2))) {
...
int compare_derived_type_name(TYPE_DESC tp1, TYPE_DESC tp2) {
  ...
  if (debug_flag) {
    fprintf(debug_fp, "   left is '%s', right is '%s'\n",
            sym1 ? SYM_NAME(sym1) : "(null)", sym2 ? SYM_NAME(sym2) : "(null)");
  ...
  FOREACH_ID(dummy_arg, dummy_args) {
    ...
    if (tbp_tp != NULL) {
      if (TYPE_BOUND_PROCEDURE_TYPE_PASS_ARG(tbp_tp)) {
        ...
        /* If the PASS argument is not specified, the first arugment is the PASS
         * argument*/
    ...
    if (!type_is_match_for_argument(ID_TYPE(dummy_arg), EXPV_TYPE(actual_arg),
                                    compare_rank, issue_error)) {
      return FALSE;
    }
