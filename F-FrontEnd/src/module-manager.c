  ...
  while (tp != NULL) {
    if (TYPE_TAGNAME(tp)) {
      ...
      if (tagname->use_assoc == NULL) {
        tagname->use_assoc =
            XMALLOC(struct use_assoc_info *, sizeof(*(id->use_assoc)));
...
#define AVAILABLE_ID(id)                                                       \
  (ID_CLASS(id) == CL_NAMELIST ||                                              \
   (TRUE &&                                                                    \
    (ID_CLASS(id) == CL_VAR || ID_CLASS(id) == CL_ENTRY ||                     \
     ID_CLASS(id) == CL_PARAM || ID_CLASS(id) == CL_CONTAINS ||                \
     ID_CLASS(id) == CL_TAGNAME || ID_CLASS(id) == CL_UNKNOWN ||               \
     ID_CLASS(id) == CL_GENERICS || ID_CLASS(id) == CL_PROC) &&                \
...
static void intermediate_file_name(char *filename, size_t len,
  ...
  if (MODULE_IS_MODULE(mod)) {
    snprintf(tmp, sizeof(tmp), "%s.%s", SYM_NAME(MODULE_NAME(mod)), extension);
  } else { /* mod is submodule */
    snprintf(tmp, sizeof(tmp), "%s:%s.%s", SYM_NAME(SUBMODULE_ANCESTOR(mod)),
...
int export_module(SYMBOL sym, ID ids, expv use_decls) {
  ...
  ID id;
  struct module *mod =
      generate_current_module(sym, NULL, ids, use_decls, FALSE);
  if (mod) {
    ret = export_xmod(mod);
  } else {
    return FALSE;
