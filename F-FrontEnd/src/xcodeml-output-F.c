...
static int outf_type_shape(XcodeMLNode *shape) {
  ...
  FOR_ITEMS_IN_XCODEML_LIST(lp, shape) {
    ...
    if ((strcmp("arrayIndex", tag) == 0) || (strcmp("indexRange", tag) == 0)) {
...
int outf_decl(char *type_signature, char *symbol, XcodeMLNode *value,
  ...
  if (type_isPrimitive(type_signature)) {
    ...
    int is_private =
        GET_IS_PRIVATE(GET_CONTENT(first)) || GET_IS_PRIVATE(GET_CONTENT(last));
    ...
    if (force == FALSE && is_private_parameter == FALSE && is_use_sym == FALSE)
      return FALSE;
    ...
    if (func_org == NULL) { /* not for function type. */
      ...
      if (outputed == FALSE)
        return FALSE;
      outputed |=
          outf_type_attribute(GET_CONTENT(first), GET_CONTENT(last), outputed);
      outputed |= outf_type_shape(GET_CONTENT(first));
    } else { /* for function type. */
      outputed |= outf_basic_type(GET_CONTENT(last), GET_TAGNAME(last));
      outputed |= outf_type_attribute(GET_CONTENT(func_org), GET_CONTENT(last),
                                      outputed);
